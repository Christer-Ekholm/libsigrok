...
static uint8_t samplerateToReg(uint64_t samplerate)
{
	const uint64_t sampleRatesValues[] = {SAMPLERATE_VALUES};
	const uint8_t  sampleRatesRegs[]   = {SAMPLERATE_REGS};
	uint32_t i;

	for(i = 0; i < ARRAY_SIZE(sampleRatesValues); i++)
	{
		if(sampleRatesValues[i] == samplerate)
		{
			return sampleRatesRegs[i];
		}
	}

	sr_err("Failed to convert samplerate: %" PRIu64, samplerate);

	return sampleRatesRegs[ARRAY_SIZE(sampleRatesValues) - 1];

}

static uint8_t voltageToReg(uint8_t state)
{
	const uint8_t  vdivReg[]   = {VDIV_REG};

	if(state < ARRAY_SIZE(vdivReg))
	{
		return  vdivReg[state];
	}
	else
	{
		sr_err("Failed to convert vdiv: %d", state);
		return vdivReg[ARRAY_SIZE(vdivReg) - 1];
	}
}

static int write_control(const struct sr_dev_inst *sdi, control_requests reg, uint8_t value)
{
	...
	if((ret = libusb_control_transfer(usb->devhdl, LIBUSB_REQUEST_TYPE_VENDOR, (uint8_t)reg,
				      0, 0, &value, 1, 100)) <= 0)
	{
...
SR_PRIV int hantek_6xxx_update_samplerate(const struct sr_dev_inst *sdi)
{
	...
	sr_dbg("update samplerate %d", samplerateToReg(devc->samplerate));

	return write_control(sdi, SAMPLERATE_REG, samplerateToReg(devc->samplerate));
}
...
SR_PRIV int hantek_6xxx_update_vdiv(const struct sr_dev_inst *sdi)
{
	...
	sr_dbg("update vdiv %d %d",voltageToReg(devc->voltage[0]), voltageToReg(devc->voltage[1]));

	ret1 = write_control(sdi, VDIV_CH1_REG, voltageToReg(devc->voltage[0]));
	ret2 = write_control(sdi, VDIV_CH2_REG, voltageToReg(devc->voltage[1]));
